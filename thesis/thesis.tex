...
Let $\mathtt{react} = (\mathtt{lhs}, \mathtt{rhs}, \nu)$ be a reaction and $\mathtt{M}$ be a multiset. The \emph{reaction application} on the multiset $\mathtt{M}$, written $\mathtt{react}(\mathtt{M})$, is a pair $(\mathtt{M} - \mathtt{lhs} + \mathtt{rhs}, \nu(\mathtt{M}))$ if $\mathtt{M} - \mathtt{lhs} > 0$; $(\mathtt{M}, 0)$ otherwise.
\end{definition}
...
A rule $\rho \in 2^\Omega$ is defined as a set of reactions. By $\mathtt{react}_i \in \mathcal{D}(\rho)$ we denote a reaction from the domain $\mathcal{D}$ of rule $\rho$ with index $i \in \mathbb{N}$. $\mathtt{I}_\rho$ denotes the set of all reaction indices from the domain $\mathcal{D}(\rho)$.
\end{definition}

A rule is a generalisation of multiple reactions. Instead of a single possible way of application as in the case of reaction, there are multiple outcomes of the application on a given multiset. 

\begin{definition}{Rule application}
Let $\rho$ be a rule and $\mathtt{M}$ be a multiset. The \emph{rule application} on the multiset $\mathtt{M}$, written $\rho(\mathtt{M})$, is a set $\{ \mathtt{react}_i(\mathtt{M}) ~|~ i \in \mathtt{I}_\rho \}$ of all reaction applications from the domain of the rule.
\end{definition}
