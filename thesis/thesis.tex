...
Let $\omega = (\mathtt{lhs}, \mathtt{rhs}, \nu)$ be a reaction and $\mathtt{M}$ be a multiset. The \emph{reaction application} on the multiset $\mathtt{M}$, written $\omega(\mathtt{M})$, is a pair $(\mathtt{M} - \mathtt{lhs} + \mathtt{rhs}, \nu(\mathtt{M}))$ if $\mathtt{M} - \mathtt{lhs} > 0$; $(\mathtt{M}, 0)$ otherwise.
\end{definition}
...
A rule $\rho \in 2^\Omega$ is defined as a set of reactions. By $\omega_i \in \mathcal{D}(\rho)$ we denote a reaction from the domain $\mathcal{D}$ of rule $\rho$ with index $i \in \mathbb{N}$. $\mathtt{I}_\rho$ denotes the set of all reaction indices from the domain $\mathcal{D}(\rho)$.
\end{definition}

A rule is a generalisation of multiple reactions. Instead of a single possible way of application as in the case of reaction, there are multiple outcomes of the application on a given multiset. By $\Sigma$ we denote the set of all rules.

\begin{definition}{Rule application}
Let $\rho$ be a rule and $\mathtt{M}$ be a multiset. The \emph{rule application} on the multiset $\mathtt{M}$, written $\rho(\mathtt{M})$, is a set $\{ \omega_i(\mathtt{M}) ~|~ i \in \mathtt{I}_\rho \}$ of all reaction applications from the domain of the rule.
\end{definition}

Given a multiset, applying a rule on it produces a set of \emph{multiset-rate} pairs. This is caused by the fact all possible reactions represented by the rule can possible produce a different outcome (some of them cannot be applied due to non-negative number of repetitions law in multisets).
...
A rule-based model is composed from two parts -- the initial state, describing the number of occurrences of individual species in the default situation, and the set of rules which defines how the species can interact.
...
In order to ...idk what..., we define \emph{explicit semantics}/discrete semantics of rule-based models. 

- or: as an example of semantics...

\begin{definition}{Quantitative Labelled Transition System}
A Quantitative Labelled Transition System (qLTS) is defined as a triple $(\mathcal{S}, \mathcal{L}, \mapsto)$ where $\mathcal{S}$ is a set of states, $\mapsto: \mathcal{S} \times \mathcal{S}$ is a transition relation, and $\mathcal{L}: \mathcal{S} \times \mathcal{S} \rightarrow \mathbb{Q}$ is a labelling function which assigns each transition from $\mapsto$ a rational number.
\end{definition}

Quantitative Labelled Transition System is a natural representation of explicit semantical meaning of rule-based model. ...

\begin{definition}{Semantics of rule-based model}
Let $\mu = (\mathcal{R}, \mathtt{M}_0)$ be a rule-based model. The semantics of the model are given in terms of qLTS $(\mathcal{S}, \mathcal{L}, \mapsto)$. It is obtained by transitive closure of rule application starting from $\mathtt{M}_0$. The nodes of qLTS represent the multisets, transition relation represents the applied rules, and quantitative labels represent the evaluated rate functions.
\end{definition}

explanation that many possible semantics are possible? how to compactly describe it?

- interpretation can differ? (from qLTS to ...)

\begin{definition}{Stochastic simulation}
bla
\end{definition}

\begin{definition}{Some logic}
bla
\end{definition}
