...
- Prism}
 ...
 For example, one can be interested in bifurcation analysis~\cite{benevs2017discrete} or attractor analysis~\cite{benevs2018fully} in ODEs; robustness analysis in continuous-time Markov chains~\cite{vceska2014robustness}; parameter synthesis in parametric discrete-time Markov chains~\cite{daws2004symbolic} etc.
...
Basically, two different ways of qualitative net analysis can be distinguished, first,
the static analysis, which considers the whole state space without constructing it, and
second, the dynamic analysis, which does construct the full or partial state space for
deciding the properties of interest.
